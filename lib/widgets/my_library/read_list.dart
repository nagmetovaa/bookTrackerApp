...
class ParentReadList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: Container(
        child: ReadList(playlistBloc: playlistBloc),
      ),
    );
  }
}
...
class ReadList extends StatefulWidget{
  ...
  ReadList({required this.playlistBloc});
              ...
              return CustomScrollView(
                    ...
                    slivers: [
                      SliverList(delegate: SliverChildBuilderDelegate(
                          (context, index) {
                            if (state.loadedPlaylist?[index].playlist == '1') {
                              return ListTile(
