...
class ParentReadingList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: Container(
        child: ReadingList(playlistBloc: playlistBloc),
      ),
    );
  }
}
...
class ReadingList extends StatefulWidget{
  ...
  ReadingList({required this.playlistBloc});
            ...
            return CustomScrollView(
              ...
              slivers: [
                SliverList(delegate: SliverChildBuilderDelegate(
                      (context, index) {
                    if (state.loadedPlaylist?[index].playlist == '2') {
                      return ListTile(
